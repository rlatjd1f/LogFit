...
Node.js 22 이상, JDK 21, Android SDK 36/36.1 이상이 필요합니다.
...
실행 중인 에뮬레이터/기기에서 계측 테스트까지 수행하려면 다음 명령을 사용합니다.
...
SDK 36 대상의 테스트용 릴리스 AAB 전체 검사는 다음 명령으로 수행합니다.

```bash
npm run release:check
```

실제 업로드용 서명 빌드 전에 `android/keystore.properties.example`을 `android/keystore.properties`로 복사하고 업로드 키 정보를 입력합니다. 키와 비밀번호 파일은 Git에서 제외됩니다.

```bash
npm run release:bundle:signed
```

스토어 문구, 개인정보처리방침 배포, Data safety, Foreground Service 선언, 이미지 자산과 출시 체크리스트는 `docs/play-store/`에 정리되어 있습니다.
